...
const loaiID = uList[uList.length - 2];
const danhmucID = uList[uList.length - 1];

let apiSPbyDM = `http://127.0.0.1/Daily/api/dssanphambydm/${loaiID}/${danhmucID}`;
...
let dataSPbyDM = fetch(apiSPbyDM)
.then((res) => res.json())
.then((dataSPbyDM) => {
    let content = ``;
    dataSPbyDM.forEach((data)=>{
        content += `
        <article class="product">
            ...
            <div class="product-content">
                ...
                <div class="product-content-price">${formatCash(data.gia)}đ</div>`
...
$(document).on("click", ".filter-item__title", function (event) {
    ...
    $(document).on("click", ".search-product-btn", function (event) {
        ...
        var minPrice = returnformatCash(document.querySelector("#minPrice").value);
        var maxPrice = returnformatCash(document.querySelector("#maxPrice").value);
        ...
        if (valueChooseOption != 0 || minPrice != "" || maxPrice != "") {
            ...
            .then((dataLoaiSanPhamS) => {
                let dataProductS = fetch(apiSPbyDM)
                .then((res) => res.json())
                ...
                .then((dataProductS) => {
                    ...
                    dataLoaiSanPhamS.forEach((category) => {
                        ...
                        if (a != null){  // a = $("#search-category-input").select2("val");
